...
def read_excel_data(file_path: Path, sheet_name: str | None=None) -> pd.DataFrame:
...
def analyze_excel_data(df: pd.DataFrame) -> dict:
    """Analyze Excel data."""
    return {'shape': df.shape, 'columns': df.columns.tolist(), 'dtypes': df.dtypes.to_dict(), 'summary': df.describe().to_dict() if len(df.select_dtypes(include=[np.number]).columns) > 0 else {}}
...
def plot_excel_analysis(df: pd.DataFrame, numeric_col: str, title: str, output_path: Path, plot: bool=False):
    ...
    if numeric_col in df.columns and df[numeric_col].dtype in ['int64', 'float64']:
        ...
        if len(numeric_cols) > 0:
            ...
            ax.text(0.5, 0.5, 'No numeric data to plot', ha='center', va='center', transform=ax.transAxes, fontsize=14)
    ax.set_xlabel('Index')
    ax.set_ylabel('Value')
