...
def main():
    parser = argparse.ArgumentParser(
        description="Integrating Excel and Python for Business Analytics"
    )
    parser.add_argument("--config", type=Path, default=None, help="Path to config file")
    parser.add_argument(
        "--excel-path", type=Path, default=None, help="Path to Excel file"
    )
    parser.add_argument(
    ...
    if args.excel_path and args.excel_path.exists():
        ...
        df = read_excel_data(
            Path(config["data"]["source"]), config["data"]["sheet_name"]
        )
        ...
        values = (
            100
            + 10 * np.sin(np.arange(config["data"]["n_rows"]) / 10)
            + np.random.normal(0, 5, config["data"]["n_rows"])
        )
        df = pd.DataFrame(
            {
                "Date": dates,
